...
def resnet101(pretrained=False, num_groups=None, weight_std=False, **kwargs):
    ...
    if pretrained:
        ...
        if num_groups and weight_std:
            pretrained_dict = torch.load('/pretrained/R-101-GN-WS.pth.tar')
            overlap_dict = {k[7:]: v for k, v in pretrained_dict.items() if k[7:] in model_dict.keys()}
            assert len(overlap_dict) == 312
        elif not num_groups and not weight_std:
            pretrained_dict = model_zoo.load_url(model_urls['resnet101'])
            overlap_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict.keys()}
        else:
