...
def update_dockerfile(version, root_dir):
    """Update TAO_TOOLKIT_VERSION in Dockerfile.release (optional - skipped if not found)."""
    filepath = os.path.join(root_dir, "release", "docker", "Dockerfile.release")
    
    if not os.path.exists(filepath):
        print(f"ℹ Skipping Dockerfile.release update (not found at {filepath})")
        return False
    
    print(f"Updating release/docker/Dockerfile.release...")
    ...
    with open(filepath, 'w') as f:
        f.write(content)
    print(f"✓ Updated release/docker/Dockerfile.release")
    return True
...
def verify_updates(version, root_dir):
    ...
    # Check Dockerfile.release (optional)
    dockerfile_path = os.path.join(root_dir, "release", "docker", "Dockerfile.release")
    if os.path.exists(dockerfile_path):
        with open(dockerfile_path, 'r') as f:
            content = f.read()
        if f'ENV TAO_TOOLKIT_VERSION="{version}"' not in content:
            issues.append("release/docker/Dockerfile.release: TAO_TOOLKIT_VERSION not updated correctly")
    ...
    try:
        ...
        if success:
            ...
            if dockerfile_updated:
                files_updated.append("release/docker/Dockerfile.release")
