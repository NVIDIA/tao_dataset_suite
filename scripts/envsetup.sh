...
TAO data service build environment set up.

The following environment variables have been set:

  NV_TAO_DS_TOP       $NV_TAO_DS_TOP

The following functions have been added to your environment:

  tao_ds                 Run command inside the container.
EOF
    fi
}

function _check_tao_ds_requirements(){
    warnings=()
...
_check_tao_ds_requirements || echo -e "\033[1;31mBuild environment setup failed.\033[0m"
